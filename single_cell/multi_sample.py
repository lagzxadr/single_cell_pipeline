...
def load_library_data(yamldata, key, caller):
    for sample_id, sample_data in yamldata[key].items():
        for library_id, variant_calls in sample_data.items(): 
            yield sample_id, library_id, variant_calls[caller]

def load_variant_calls(yamldata, caller):
    variant_files = {}
    for sample_id, library_id, variant_call in load_library_data(yamldata, 'variant_calls', caller):
         variant_files[sample_id, library_id] = variant_call
...
def generate_meta_files(normal_sample_id, normal_libraries, tumour_cell_bams, args):
    ...
    svtyper_dir=args["svtyper_output"]
...
def multi_sample_pipeline(args):
    ...
    #load variant calls
    if args["variant_calls"]:
        variant_call_data = helpers.load_yaml(args["variant_calls"])
        lumpy_variant_calls = load_variant_calls(variant_call_data, "lumpy")
        destruct_variant_calls = load_variant_calls(variant_call_data, "destruct")
    ...
    workflow = create_multi_sample_workflow(
        ...
        varcall_dir=args["variants_output"],
        svtyper_dir=args["svtyper_output"],
        input_lumpy_variant_calls = lumpy_variant_calls,
        input_destruct_variant_calls = destruct_variant_calls,
...
def create_multi_sample_workflow(
        ...
        varcall_dir=None,
        svtyper_dir=None,
        input_lumpy_variant_calls = None,
        input_destruct_variant_calls = None,
        normal_sample_id='normal',
):
    """ Multiple sample pseudobulk workflow. """

    baseimage = config['genotyping']['docker']['scp']
    ...
    if lumpy_dir:
        ...
        workflow.subworkflow(
            ...
            func='single_cell.workflows.lumpy.lumpy_multi_sample_workflow2',
    ...
    #performs genotyping on lumpy + destruct variant calls
    if svtyper_dir and input_lumpy_variant_calls and input_destruct_variant_calls:
        genotyping_pipeline_config = config['genotyping']
        output_template = {}
        ...
        for annotation in annotations: 
            output_template[annotation] =  os.path.join(svtyper_dir, '{sample_id}', '{library_id}', annotation+".csv")

        output_template["merged"] = os.path.join( svtyper_dir, '{sample_id}', '{library_id}', 'lumpy_destruct_merged.csv')
        ref = genotyping_pipeline_config["data"]["reference"]
        workflow.subworkflow(
            name='genotype',
            func='single_cell.workflows.genotyping.genotyping',
            axes=('sample_id', 'library_id', ),
            ctx={'docker_image': genotyping_pipeline_config['docker']['scp']},
            args=(
                ref,
                mgd.InputFile('bam_input', 'sample_id', 'library_id',  'cell_id', extensions=['.bai'],
                              axes_origin=[], fnames=tumour_cell_bams),
                mgd.InputFile('lumpy_csv_gz', 'sample_id', 'library_id', axes_origin=[], fnames=input_lumpy_variant_calls),
                mgd.InputFile('destruct_csv_gz', 'sample_id', 'library_id', axes_origin=[], fnames=input_destruct_variant_calls),
